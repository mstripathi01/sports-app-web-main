...
const VideoLevel = () => {
  ...
  const exeriseFormUrl = `${PATH_DASHBOARD.level.levels}/exercise_form`;
  const detailUrl = `${PATH_DASHBOARD.level.levels}/detail`;
  const videoCountUrl = `${PATH_DASHBOARD.level.video_count}`;
  const runningUsersUrl = `${PATH_DASHBOARD.level.running_users}`;
  const completeUsersUrl = `${PATH_DASHBOARD.level.complete_users}`;
  const actionUrl = "#";
      ...
      renderCell: ({ row }) => (
        <Box sx={{ width: "100%", textAlign: "center" }}>
          <Label
            ...
            variant="soft"
            component={Link}
            href={`${PATH_DASHBOARD.level.exercise}`}
